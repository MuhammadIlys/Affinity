                                ...
                                @if ($payouts)
                                    @foreach ($payouts as $payout)
                                        <tr class="text-start">
                                            ...
                                            <td>
                                                <a href="{{ route('request.approve', encrypt($payout->id)) }}"
                                                    class="btn btn-sm btn-success mb-1"
                                                    onclick="return confirm('Are you sure you want to Approve this request?')">
                                                    Approve
                                                </a>
                                                <a href="javascript:void(0)" class="btn btn-sm btn-primary mb-1 edit-btn"
                                                    data-id="{{ $payout->id }}" data-amount="{{ $payout->total_amount }}"
                                                    data-url="{{ route('request.update', encrypt($payout->id)) }}">
                                                    Edit
                                                </a> {{-- <a href="{{ route('request.reject',encrypt($payout->id)) }}" class="btn btn-sm btn-warning mb-1">Reject</a> --}}
